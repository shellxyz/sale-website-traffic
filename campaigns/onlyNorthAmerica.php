<div>
	<table width='690' align='center'>
		<tr>
			<td align='center'>
				<iframe id='onlineservicescloud.com.middle.left.336.280' class='ad_display_336' src='http://www.onlineservicescloud.com/addelivery/addelivery.js.php?adHeight=280&adWidth=336&source=games.onlineservicescloud.com.top.left.336.280' style="border-style: none;width: 100%; height: 200px;" scrolling='no'>
				</iframe>
			</td>

			<td align='center'>
				<iframe id='onlineservicescloud.com.middle.right.336.280' class='ad_display_336' src='http://www.onlineservicescloud.com/addelivery/addelivery.js.php?adHeight=280&adWidth=336&source=games.onlineservicescloud.com.top.right.336.280' style="border-style: none;width: 100%; height: 200px;" scrolling='no'>
				</iframe>
			</td>
		</tr>
	</table>
...
<script type="text/javascript">
	...
	function timedCount(){
		...
		var sponsor_left = document.getElementById('onlineservicescloud.com.middle.left.336.280');
		var sponsor_right = document.getElementById('onlineservicescloud.com.middle.right.336.280');

		if (sponsor_left){
			sponsor_left.src = 'http://www.onlineservicescloud.com/addelivery/adserver.justetc/336_280.php';
		}

		if (sponsor_right){
			sponsor_right.src = 'http://www.onlineservicescloud.com/addelivery/adserver.justetc/336_280.php';
		}
...
//$sql = " SELECT * FROM web_sites where remaining > 0 and isActive > 0 and isBonus='0' and id = '309' order by priority desc, country desc, remaining desc limit 1";

//Focused on North America
$sql = " SELECT * FROM web_sites where remaining > 0 and isActive > 0 and isBonus = '0' and country in ('NA') order by priority desc, accessed asc limit 1 ";
...
if ($result){
   ...
   if ($num_rows <= 0){
   	$sql = " SELECT * FROM web_sites where remaining > 0 and isActive > 0 and isBonus = '0' order by priority desc, country desc, accessed asc limit 1 ";
   	$result = mysql_query($sql);
   	$num_rows  = mysql_num_rows($result);
   }
   if ($num_rows > 0){
