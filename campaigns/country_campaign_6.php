...
<?php
	...
	$sql = "SELECT * FROM web_sites where remaining > 0 and isActive > 0 and isBonus = '0' and country in ('USA') and is_freezed = 0 order by priority desc, accessed asc limit 1 ";
	$sql = "SELECT *  FROM web_sites WHERE clientId = '184' and remaining > 0 and isActive > 0 and isBonus = '0' and country in ('USA') and is_freezed = 0 order by priority desc, accessed asc limit 1 ";
...
<iframe src='<?php echo $justetcUrl; ?>' width='95%' height='180'>
</iframe>
...
<?php
         ...
         while ($i < $num_rows){
             ...
             if ($deduct){
	             ...
	             if ($remaining <= 1){
		            ...
		            $send = mail('traffic_support@onlineservicescloud.com','One of your campaigns just got finished',$body);
		            $send = mail($email,'One of your campaigns just got finished',$body);
   	...
   	$send = mail('sales@onlineservicescloud.com','You may need to stop an Adbrite campaign','You may need to stop an Adbrite campaign\n\n'.$information.'\n\nlooks like no traffic to deliver for the day');
