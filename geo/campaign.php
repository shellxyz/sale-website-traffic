...
<?php
	...
	$sql = " SELECT * FROM web_sites where remaining > 0 and isActive > 0 and isBonus = '0' and country in ('$country', 'All') and is_freezed = 0 order by country desc, priority desc, accessed asc limit 1 ";
	...
	if ($result){
	   ...
	   if ($num_rows > 0){
	   		...
	   		$sqlIp = " insert into   vshop_ip_and_display_track (ip,accessed, country) values ('$ip',$currentTime,'$country')
				ON DUPLICATE KEY UPDATE accessed='$currentTime', times=times+1";

			mysql_query($sqlIp);

	   		$justetcUrl = "http://".$_SERVER['SERVER_NAME'].$_SERVER['PHP_SELF'].'?';
            if (isset($_SERVER['QUERY_STRING']) && ($_SERVER['QUERY_STRING']!="")){
            	$justetcUrl .= $_SERVER['QUERY_STRING'].'&isCampaign=1';
            }else{
            	$justetcUrl .= 'isCampaign=1';
            }
?>

<iframe src='<?php echo $justetcUrl; ?>' width='95%' height='280'>
</iframe>
